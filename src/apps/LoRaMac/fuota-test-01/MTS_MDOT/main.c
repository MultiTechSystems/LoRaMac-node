...
static volatile size_t UnfragmentedDataRcvd = 0;
...
int main( void )
{
    ...
    while( 1 )
    {
        ...
        while(!IsFifoEmpty(&Uart1.FifoTx)) { }

        if ( IsFileTransferDone && ( unfragPrintIndex < UnfragmentedDataRcvd ) )
        {
            UartPutBuffer(&Uart2, &UnfragmentedData[unfragPrintIndex], 512);
            unfragPrintIndex += 512;
        }
...
static uint8_t FragDecoderWrite( uint32_t addr, uint8_t *data, uint32_t size )
{
    ...
    if (addr + size > UnfragmentedDataRcvd)
    {
        UnfragmentedDataRcvd = addr + size;
    }
