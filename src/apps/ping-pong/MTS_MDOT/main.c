...
{
    uint16_t reg;
    static uint8_t radio_regs[0x047 + 7] = {0};
    if (radio_regs[1] == 0)
    {
        for(reg = 0x00; reg < 0x047; reg++ )
        {
            radio_regs[reg] = reg;
        }
    ...
    for( reg = 0x00; reg < sizeof(radio_regs); reg++ )
    {
        uint8_t reg_val = SX1272Read(radio_regs[reg]);
        printf("addr %02X : %02X \r\n", radio_regs[reg], reg_val);
        while(!IsFifoEmpty(&Uart2.FifoTx)) { }
    }
}
...
int main( void )
{
    ...
    #error "Please define a frequency band in the compiler options."
#endif
    TimerTime_t time = TimerGetCurrentTime();
    printf("%lu\n\r", time);
    TimerTime_t time2;
...
void OnTxDone( void )
{
    ...
    State = TX;
    GpioWrite( &ind, 1 );
    ind = (ind.pin == Led1.pin) ? Led2: Led1;
}
